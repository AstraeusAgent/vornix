...
mod tests {
    ...
    async fn migrate_extracts_plaintext_keys() {
        let dir =
            std::env::temp_dir().join(format!("sable-migration-test-{}", std::process::id()));
    ...
    async fn migrate_skips_keys_already_in_vault() {
        let dir =
            std::env::temp_dir().join(format!("sable-migration-skip-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn migrate_removes_file_when_all_keys_are_secrets() {
        let dir =
            std::env::temp_dir().join(format!("sable-migration-rm-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn migrate_handles_empty_dir() {
        let dir =
            std::env::temp_dir().join(format!("sable-migration-empty-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
