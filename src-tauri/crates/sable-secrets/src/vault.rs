...
/// Known provider key names that Sable manages.
...
const KEYRING_SERVICE: &str = "sable";

/// Salt mixed into the machine-specific passphrase derivation.
const PASSPHRASE_SALT: &str = "sable-secrets-v1";
...
impl SecretsVault {
    ...
    /// 2. Encrypted file at `~/.sable/secrets.enc`.
    ...
    fn probe_keyring() -> Result<()> {
        let probe_key = format!("sable/__vault_probe_{}", std::process::id());
...
///
/// Entry naming: service = `"sable"`, user = `<key>`.
...
/// JSON file at `~/.sable/secrets.enc`.
///
...
impl EncryptedFileBackend {
    ...
    fn new() -> Result<(Self, PathBuf)> {
        ...
        let sable_dir = home.join(".sable");
        std::fs::create_dir_all(&sable_dir)
            .with_context(|| format!("Creating {}", sable_dir.display()))?;
        ...
        let path = sable_dir.join("secrets.enc");
...
mod tests {
    ...
    async fn encrypted_file_roundtrip() {
        let dir = std::env::temp_dir().join(format!("sable-secrets-test-{}", std::process::id()));
