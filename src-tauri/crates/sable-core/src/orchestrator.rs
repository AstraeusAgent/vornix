...
use sable_tools::{PermissionDecision, PermissionRequest, ToolCall, ToolResult};
...
/// [`sable_providers::ChatStream`](sable_providers::ChatStream), but backed
    ...
    pub async fn process_user_message(
        ...
        loop {
            ...
            for acc in &tool_calls_snapshot {
                // Build a sable_tools::ToolCall for the executor.
                ...
                match decision {
                    ...
                    PermissionDecision::RequiresApproval => {
                        ...
                        let tier =
                            sable_tools::PermissionPolicy::classify(&tool_call);
