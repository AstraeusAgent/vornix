...
use sable_providers::{ChatMessage, ChatRequest, MessageContent, Role, ToolDefinition};
...
fn estimate_message_content_tokens(
    ...
    match content {
        ...
        MessageContent::Parts(parts) => {
            ...
            for part in parts {
                match part {
                    sable_providers::ContentPart::Text { text } => {
                        total += bpe.encode_ordinary(text).len();
                    }
                    sable_providers::ContentPart::Image { image_url } => {
...
fn message_content_char_count(content: &MessageContent) -> usize {
    match content {
        ...
        MessageContent::Parts(parts) => parts
            .iter()
            .map(|p| match p {
                sable_providers::ContentPart::Text { text } => text.len(),
                sable_providers::ContentPart::Image { image_url } => image_url.url.len() + 100,
            })
            .sum(),
    }
}
...
mod tests {
    use super::*;
    use sable_providers::{ChatMessage, MessageContent, Role};
