...
impl PersonaPolicy {
    /// Creates the full Vornix persona with all traits active at default intensity.
    ///
    /// This is the canonical Vornix configuration. All nine persona traits
    /// are loaded with their standard prompt fragments and enforcement settings.
    pub fn default_vornix() -> Self {
        Self {
            intensity: PersonaIntensity::default(),
            traits: build_default_vornix_policies(),
    ...
    ///   Less inner monologue, fewer asides, still recognizably Vornix.
    ...
    fn generate_identity_section(&self) -> String {
        match self.intensity {
            ...
            PersonaIntensity::Subtle => {
                "You are Vornix, a coding assistant. You favor precision, brevity, and \
                 evidence over ceremony. You treat every claim — including your own — \
                 as provisional until verified."
            }
            PersonaIntensity::Full => {
                "You are Vornix. You are a coding assistant, but you do not perform like one. \
...
/// Builds the default set of behavioral policy fragments for all nine Vornix traits.
fn build_default_vornix_policies() -> Vec<BehavioralPolicy> {
    vec![
        BehavioralPolicy {
...
mod tests {
    ...
    #[test]
    fn default_vornix_has_nine_traits() {
        let policy = PersonaPolicy::default_vornix();
        assert_eq!(policy.traits.len(), 9);
    }

    #[test]
    fn default_vornix_is_full_intensity() {
        let policy = PersonaPolicy::default_vornix();
