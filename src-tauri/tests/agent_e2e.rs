...
//! process (sable-fs over stdio JSON-RPC), real SQLite persistence, and files
...
use sable_app::agent::{run_agent_turn, AgentDeps};
...
async fn agent_builds_web_os_end_to_end() {
    ...
    // --- Real MCP server: sable-fs as a child process over stdio ---
    let servers_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .map(|p| p.join("mcp-servers"))
        .unwrap();
    let script = servers_dir.join("sable-fs").join("index.cjs");
    assert!(script.exists(), "sable-fs script missing at {}", script.display());

    let mut mcp = McpManager::new();
    let mut env = HashMap::new();
    env.insert(
        "SABLE_WORKSPACE".to_string(),
        workspace.display().to_string(),
    );
    mcp.add_server(McpServerConfig {
        name: "sable-fs".to_string(),
    ...
    .await
    .unwrap();
    mcp.connect_server("sable-fs")
        .await
        .expect("sable-fs MCP handshake failed");
    ...
    let task = "Build a simple web OS in this workspace consisting of three files: \
        ...
        (Files, Terminal, About), a taskbar with a clock, draggable and closable windows, \
