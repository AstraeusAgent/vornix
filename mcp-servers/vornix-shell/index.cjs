#!/usr/bin/env node
// vornix-shell: MCP server for shell/process control
// Speaks JSON-RPC 2.0 over stdio

const { spawn, execSync } = require("child_process");
const readline = require("readline");

const WORKSPACE = process.env.VORNIX_WORKSPACE || process.cwd();
...
function handleRequest(req) {
  ...
  if (method === "initialize") {
    return ok(id, {
      ...
      serverInfo: { name: "vornix-shell", version: "0.1.0" },
    });
  }
...
rl.on("line", (line) => {
  try {
    ...
    process.stderr.write(`vornix-shell: parse error: ${e.message}\n`);
  }
});
rl.on("close", () => process.exit(0));

process.stderr.write("vornix-shell: started\n");