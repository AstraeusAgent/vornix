#!/usr/bin/env node
// vornix-thinking: MCP server for sequential thinking scratchpad
...
function handleRequest(req) {
  ...
  if (method === "initialize") {
    return ok(id, {
      ...
      serverInfo: { name: "vornix-thinking", version: "0.1.0" },
    });
  }
...
rl.on("line", (line) => {
  try {
    ...
    process.stderr.write(`vornix-thinking: parse error: ${e.message}\n`);
  }
});
rl.on("close", () => process.exit(0));

process.stderr.write("vornix-thinking: started\n");