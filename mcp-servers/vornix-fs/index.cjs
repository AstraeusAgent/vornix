#!/usr/bin/env node
// vornix-fs: MCP server for filesystem operations
...
function handleRequest(req) {
  ...
  if (method === "initialize") {
    return ok(id, {
      ...
      serverInfo: { name: "vornix-fs", version: "0.1.0" },
    });
  }
...
rl.on("line", (line) => {
  try {
    ...
    process.stderr.write(`vornix-fs: parse error: ${e.message}\n`);
  }
});
rl.on("close", () => process.exit(0));

process.stderr.write("vornix-fs: started\n");